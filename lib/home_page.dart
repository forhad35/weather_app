...
class _HomePageState extends State<HomePage> {
  ...
  fetchData() async {
    try {
      ...
      if (_cityName.text.isNotEmpty) {
        try {
          _weather = await wf.currentWeatherByCityName(_cityName.text);
          _cityName.clear();
        } catch (e) {
          if (kDebugMode) {
  ...
  Widget _sunRiseSunSet() {
    return Row(
      ...
      children: [
        Text(
          "Sunrise ${DateFormat("h:m a").format(_weather!.sunrise!)}",
          style: const TextStyle(
              color: Colors.black, fontSize: 16, fontWeight: FontWeight.w600),
          textAlign: TextAlign.center,
        ),
        Text(
          "Sunset ${DateFormat("h:m a").format(_weather!.sunset!)}",
          style: const TextStyle(
              color: Colors.black, fontSize: 16, fontWeight: FontWeight.w600),
          textAlign: TextAlign.center,
        )
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text(
                "Humidly : ${_weather!.humidity?.toStringAsFixed(0)}%",
                textAlign: TextAlign.left,
                style: const TextStyle(color: Colors.white, fontSize: 15),
              ),
            ],
